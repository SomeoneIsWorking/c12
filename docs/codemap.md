...
| Direct runtime identity and title policy | `game/runtime/c12_runtime.*` | C-12 supplies immutable executable facts to `GameRuntime`; future cohesive title owners implement native lifecycle and presentation |
| Bounded startup observation | `game/app/boot_probe.cpp` | Authenticate the title bytes, map them through psxport, bind hardware, and observe existing `executeUntilExit` continuations without inventing a native frame driver |
